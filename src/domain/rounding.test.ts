...
describe('roundDose', () => {
  it('rounds to 1 mg by default and keeps the unrounded value', () => {
    ...
    expect(result).toMatchObject({ unroundedMg: 681.795, roundedMg: 682, method: 'step' })
    expect(result.deviationPercent).toBeCloseTo(0.0301, 4)
  })

  it('rounds halves up', () => {
    expect(roundDose(2.5).roundedMg).toBe(3)
    expect(roundDose(90.5).roundedMg).toBe(91)
  })

  it('supports custom steps', () => {
    expect(roundDose(2.545, { stepMg: 0.1 }).roundedMg).toBe(2.5)
    expect(roundDose(1363.59, { stepMg: 5 }).roundedMg).toBe(1365)
  })

  it('does not snap to vials unless a tolerance is given', () => {
    expect(roundDose(487, { presentations: vials }).roundedMg).toBe(487)
  })
  ...
  it('snaps to the nearest whole-vial amount within tolerance', () => {
    ...
    expect(result.roundedMg).toBe(500)
    expect(result.method).toBe('vial')
    expect(result.steps[0]?.key).toBe('rounding.vial')
    // 412 → 400 (−2.9%)
    expect(roundDose(412, { presentations: vials, vialTolerancePercent: 5 }).roundedMg).toBe(400)
  })
  ...
  it('falls back to step rounding when vials are too far', () => {
    ...
    expect(result).toMatchObject({ roundedMg: 440, method: 'step' })
  })

  it('breaks ties upwards', () => {
    expect(roundDose(450, { presentations: vials, vialTolerancePercent: 20 }).roundedMg).toBe(500)
  })

  it('handles zero and invalid input', () => {
    expect(roundDose(0)).toMatchObject({ roundedMg: 0, deviationPercent: 0 })
