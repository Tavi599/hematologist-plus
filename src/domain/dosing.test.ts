...
describe('calculateDose — R-CHOP example', () => {
  it('calculates mg/m² doses', () => {
    ...
    expect(calculateDose({ value: 375, unit: 'mg_m2' }, context).doseAmount).toBeCloseTo(681.8, 1)
    // cyclophosphamide 750 × 1.81812 = 1363.59
    expect(calculateDose({ value: 750, unit: 'mg_m2' }, context).doseAmount).toBeCloseTo(1363.59, 2)
    // doxorubicin 50 × 1.81812 = 90.906
    expect(calculateDose({ value: 50, unit: 'mg_m2' }, context).doseAmount).toBeCloseTo(90.906, 3)
  })

  it('caps vincristine at 2 mg', () => {
    // 1.4 × 1.81812 = 2.545 → 2
    const result = calculateDose({ value: 1.4, unit: 'mg_m2', capAmount: 2 }, context)
    expect(result.baseAmount).toBeCloseTo(2.545, 3)
  ...
  it('does not cap when below the limit', () => {
    const result = calculateDose(
      { value: 1.4, unit: 'mg_m2', capAmount: 2 },
      { ...context, bsaM2: 1.2 },
    )
    expect(result.isCapped).toBe(false)
    expect(result.doseAmount).toBeCloseTo(1.68, 10)
  })
})

describe('calculateDose — other units', () => {
  it('calculates mg/kg', () => {
    expect(calculateDose({ value: 10, unit: 'mg_kg' }, context).doseAmount).toBe(700)
  })

  it('keeps flat doses', () => {
    const result = calculateDose({ value: 1400, unit: 'mg_flat' }, context)
    expect(result.doseAmount).toBe(1400)
  ...
  it('uses Calvert for AUC doses', () => {
    ...
    expect(result.doseAmount).toBeCloseTo(513.89, 2)
  ...
  it('validates dose values', () => {
    ...
    expect(() => calculateDose({ value: 1, unit: 'mg_flat', capAmount: 0 }, context)).toThrow(
      DomainInputError,
    )
...
describe('dose reduction', () => {
  it('applies the course reduction', () => {
    ...
    expect(result.doseAmount).toBeCloseTo(68.18, 2)
  ...
  it('lets a per-drug reduction replace the course reduction', () => {
    ...
    expect(result.doseAmount).toBeCloseTo(45.45, 2)
...
describe('calculateDoseVariants', () => {
  it('differs only for mg/m² doses in patients above the BSA cap', () => {
    ...
    expect(cyclo.actual.doseAmount).toBeCloseTo(1807.1, 1)
    expect(cyclo.capped.doseAmount).toBe(1500)
